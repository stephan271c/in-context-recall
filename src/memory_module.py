...
def inner_optimization_forward(
    ...
    states = _broadcast_state_tree(
        inner_optimizer.init_states(base_params), effective_batch, device
    )

    if inner_optimzer_kwargs is None:
        static_hparams = {}
    else:
        static_hparams = {k: v for k, v in inner_optimzer_kwargs.items() if k != "lr"}
        static_hparams = {
            k: (
    ...
    for t in range(seq_len):
        ...
        if isinstance(inner_loss_weight_head, nn.Module):
            loss_weight = inner_loss_weight_head(current_keys)  # shape (B, context_window)
        else:
            loss_weight = inner_loss_weight_head  # shape (context_window,)
        ...
        if isinstance(inner_lr_head, nn.Module):
            if isinstance(inner_lr_head, LearnableHyperparam):
                lr_values = inner_lr_head()  # shape (1,)
            else:
                lr_values = inner_lr_head(current_keys)  # shape (B,)
        else:
            lr_values = inner_lr_head  # float
        # of shape (B,)
