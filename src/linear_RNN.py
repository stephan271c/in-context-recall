...
class MesaLayerMemory:
    ...
    def update_fn(R_matrix: torch.Tensor, S_matrix: torch.Tensor, phi_matrix: torch.Tensor,
        ...
        # Update R_matrix: R - (Rk @ Rk^T) / denom
        R_matrix = R_matrix - outer_Rk / denom.unsqueeze(-1).unsqueeze(-1)

        # Update S_matrix: gamma * S + outer_product(value, key)
        if gamma.ndim == 0:  # scalar
            S_matrix = gamma * S_matrix + torch.einsum('bv, bk -> bvk', value, key)
        else:  # (batch_size,)
    ...
    def init_matrices(self, batch_size: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        R_matrix = self.lambda_init * torch.eye(self.key_dim).unsqueeze(0).expand(batch_size, -1, -1).clone()
